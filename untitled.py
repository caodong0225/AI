...
class Ui_MainWindow(object):
        ...
        for i in range(len(self.structure)-1):
            for k in range(self.structure[i+1]):
                for l in range(self.structure[i]):
                    self.trainer.grad[i][k][l] = float(self.centralwidget.findChild(QtWidgets.QTableWidget, "table"+str(i)).item(k,l).text())
            self.trainer.bias[i] = float(self.centralwidget.findChild(QtWidgets.QTableWidget, "table"+str(i)).item(0,l+1).text())

            self.trainw[i].append(self.trainer.grad[i])
            self.trainb[i].append(self.trainer.bias[i])
        self.lossvalue = []
        self.trainbegin = train_thread()
        self.trainbegin.send_signal.connect(self.update_table)
        self.trainbegin.setdata(inp=self.datain,expe=self.dataout,stru=self.structure,activemod=self.active,epoch=epochs,
                           batchsize=batchsize,lossmod=lossmod,learnrate=learnrate,gradmod=gradmod,trainer=self.trainer)
    ...
    def colorset(self,num):
        if num>0:
            return ((1-math.tanh(0.5*num))*255,255,255)
        else:
            return (255,(1-math.tanh(-0.5*num))*255,255)
        ...
        for i in range(len(self.structure)-1):
            ...
            for k in range(self.structure[i+1]):
                for l in range(self.structure[i]):
                    ...
                    itemcolor = self.colorset(itemvalue)
            ...
            for i in range(len(self.structure)):
                if i==0:
                    if self.structure[i]<=4:
                        ...
                        painter.setFont(QFont('SimSun',10))
                        painter.drawText(startx+i*50+8-len(str(self.structure[i])), starty-15*self.structure[i]-5, str(self.structure[i]))
                    else:
                        painter.drawText(startx+i*50-5, starty+65, "input")
                        painter.setFont(QFont('SimSun',10))
                        painter.drawText(startx+i*50+8-len(str(self.structure[i])), starty-70, str(self.structure[i]))
                else:
                    if self.structure[i]<=4:
                        painter.setFont(QFont('SimSun',15-len(self.active[i-1])))
                        painter.drawText(startx+i*50-len(self.active[i-1]), starty+15*self.structure[i], self.active[i-1])
                        painter.setFont(QFont('SimSun',10))
                        painter.drawText(startx+i*50+8-len(str(self.structure[i])), starty-15*self.structure[i]-5, str(self.structure[i]))
                    else:
                        painter.setFont(QFont('SimSun',15-len(self.active[i-1])))
...
# 继承QThread
class train_thread(QThread):  # 线程1
    global if_con
    send_signal = pyqtSignal(int,float)
    def __init__(self):
        super().__init__()
    def setdata(self,inp,expe,stru,activemod,epoch,batchsize,lossmod,learnrate,gradmod,trainer):#专门定义一个方法将主线程的参数传给子线程
        self.inp = inp
        self.expe = expe
        self.stru = stru
    ...
    def run(self):
        global if_con
        num = 0
        for i in self.trainer.train(inp=self.inp,expe=self.expe,stru=self.stru,activemod=self.activemod,epoch=self.epoch,
                           batchsize=self.batchsize,lossmod=self.lossmod,learnrate=self.learnrate,gradmod=self.gradmod):
            if if_con:
                num = num + 1
